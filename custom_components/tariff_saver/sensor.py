...
def _grade_from_dev(dev: float) -> int:
    ...
    NOTE: default thresholds; later make configurable via options.
    """
    if dev <= -20:
        return 1
    if dev <= -10:
...
def _price_for_now(coordinator: TariffSaverCoordinator) -> tuple[float | None, float | None]:
    """Return (active_price, baseline_price) for the current slot."""
    active_slots = _active_slots(coordinator)
    if not active_slots:
        return None, None

    slot = _current_slot(active_slots)
    if slot is None:
        return None, None

    active_price = slot.price_chf_per_kwh if slot.price_chf_per_kwh > 0 else None

    baseline_price = None
    baseline = _baseline_slots(coordinator)
    if baseline:
        base_map = {s.start: s.price_chf_per_kwh for s in baseline if s.price_chf_per_kwh and s.price_chf_per_kwh > 0}
        baseline_price = base_map.get(slot.start)
...
async def async_setup_entry(
    ...
    if isinstance(energy_entity, str) and energy_entity:
        ...
        def _on_energy_change(event: Event) -> None:
            ...
            active_price, baseline_price = _price_for_now(coordinator)
            if active_price is None:
                # no valid price yet; don't accumulate
                tracker.last_energy_kwh = new_val
                return
...
class _BaseTodayCostSensor(CoordinatorEntity[TariffSaverCoordinator], SensorEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        # restore last totals to avoid showing 0 after restart
...
class TariffSaverActualSavingsTodaySensor(_BaseTodayCostSensor):
    ...
    def _restore_value(self, tracker: _CostTracker, v: float) -> None:
        # We can't restore savings directly without components; ignore.
        return
